...
public class DashboardService {
    ...
    private Map<String, Integer> loadRoleDistribution() throws SQLDataException {
        ...
        String sql = "SELECT LOWER(TRIM(`role`)) AS role_name, COUNT(*) AS total FROM `user` GROUP BY LOWER(TRIM(`role`))";
        try (PreparedStatement ps = connection.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                String role = rs.getString("role_name");
                int total = rs.getInt("total");
                if ("admin".equals(role)) {
                    distribution.put("Admin", total);
                } else if ("artiste".equals(role)) {
                    distribution.put("Artistes", total);
                } else if ("amateur".equals(role)) {
                    distribution.put("Amateurs", total);
                }
            }
        } catch (SQLException e) {
    ...
    private List<String> loadRecentSignups() throws SQLDataException {
        ...
        if (table == null || dateColumn == null) {
            return List.of("Aucune inscription recente disponible.");
        }
        ...
        if (rows.isEmpty()) {
            rows.add("Aucune inscription recente disponible.");
        }
        return rows;
    }
    ...
    private List<String> loadRecentReclamations() throws SQLDataException {
        ...
        if (table == null) {
            return List.of("Aucune table reclamation detectee.");
        }
        ...
        if (textColumn != null) {
            sql.append("`").append(textColumn).append("`");
        } else {
            sql.append("NULL");
        }
        sql.append(" AS text_col");
        if (dateColumn != null) {
            sql.append(", `").append(dateColumn).append("` AS date_col");
        } else {
            sql.append(", NULL AS date_col");
        }
        ...
        if (rows.isEmpty()) {
            ...
            if (textColumn != null) {
                fallbackSql.append("`").append(textColumn).append("`");
            } else {
                fallbackSql.append("NULL");
            }
            fallbackSql.append(" AS text_col");
            if (dateColumn != null) {
                fallbackSql.append(", `").append(dateColumn).append("` AS date_col");
            } else {
                fallbackSql.append(", NULL AS date_col");
            }
            fallbackSql.append(" FROM `").append(table).append("`");
            if (dateColumn != null) {
                fallbackSql.append(" ORDER BY `").append(dateColumn).append("` DESC");
            }
            fallbackSql.append(" LIMIT 8");

            try (PreparedStatement ps = connection.prepareStatement(fallbackSql.toString()); ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    String text = safeTrim(rs.getString("text_col"));
                    if (text.isEmpty()) {
                        text = "Reclamation";
        ...
        if (rows.isEmpty()) {
            rows.add("Aucune reclamation recente disponible.");
        }
        return rows;
    }
    ...
    private List<String> loadTopArtistesFallback() throws SQLDataException {
        List<String> rows = new ArrayList<>();
        String sql = "SELECT `nom`, `prenom` FROM `user` WHERE LOWER(TRIM(`role`)) = 'artiste' ORDER BY `nom` ASC LIMIT 5";
        ...
        if (rows.isEmpty()) {
            rows.add("Aucun artiste disponible.");
        }
        return rows;
    }
        ...
        try {
            ...
            try (ResultSet rs = metaData.getTables(connection.getCatalog(), null, "%", new String[]{"TABLE"})) {
                while (rs.next()) {
                    ...
                    if (tableName != null) {
                        tables.put(tableName.toLowerCase(Locale.ROOT), tableName);
                    }
                }
            }
        } catch (SQLException e) {
    ...
    private String resolveColumnType(String tableName, String columnName) throws SQLDataException {
        try {
            ...
            try (ResultSet rs = metaData.getColumns(connection.getCatalog(), null, tableName, columnName)) {
                if (rs.next()) {
                    return safeTrim(rs.getString("TYPE_NAME"));
                }
            }
        } catch (SQLException e) {
    ...
    private String formatDateValue(Object rawDate) {
        ...
        if (rawDate instanceof java.time.LocalDateTime localDateTime) {
    ...
    public static final class DashboardData {
        ...
        private final int totalArtistes;
        private final int totalAmateurs;
        private final int totalOeuvres;
        private final int totalReclamations;
        private final int totalEvenements;
        private final List<MonthlySignupPoint> signupsByMonth;
        private final Map<String, Integer> roleDistribution;
        private final List<String> recentSignups;
        private final List<String> recentReclamations;
        private final List<String> topArtistes;
        ...
        public DashboardData(
            ...
            this.totalArtistes = totalArtistes;
            this.totalAmateurs = totalAmateurs;
            this.totalOeuvres = totalOeuvres;
        ...
        public int getTotalArtistes() {
            return totalArtistes;
        }

        public int getTotalAmateurs() {
            return totalAmateurs;
        }

        public int getTotalOeuvres() {
            return totalOeuvres;
        }
        ...
        public List<MonthlySignupPoint> getSignupsByMonth() {
        ...
        public List<String> getRecentSignups() {
            return recentSignups;
        }

        public List<String> getRecentReclamations() {
            return recentReclamations;
        }

        public List<String> getTopArtistes() {
            return topArtistes;
        }
    }
